...
class SportsViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Skip", style: .plain, target: self, action: #selector(skipProfileFilling))
    }
    
    @objc func skipProfileFilling(){
        let title = baseballButton.title(for: .normal)
        ref!.child("users").child(Auth.auth().currentUser!.uid).child(title!).setValue("KnvZfZ7v7na")
        performSegue(withIdentifier: "skipToMainFromSports", sender: self)
    }
